    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $accounts = Account::query()
                ...
                ->select(['accounts.name', 'accounts.account_number', 'accounts.note', 'accounts.id', 'accounts.account_type_id', 'accounts.ar_name', 'accounts.en_name', 'accounts.transaction_type', 'accounts.parent_id',
            ...
            return DataTables::of($accounts)
                ...
                ->editColumn('name', function ($row) {
                    $name = app()->getLocale() == 'ar' ? $row->ar_name : $row->en_name ?? $row->name;
                    if ($row->is_closed == 1) {
...
//                   return $query->select('ar_name')->from('accounts')->where($query->id)->limit(1);
                ...
                ->editColumn('transaction_type', function ($row) {
                    ...
                    return __('lang_v1.' . $transaction_type);
                })
    ...
    public function create()
    {
        ...
        $account_types = AccountType::where('business_id', $business_id)
            ...
            ->with(['sub_types'])
            ->get();
        $accounts = Account::query()->select(['id', 'name', 'en_name','ar_name', 'account_number'])->orderBy('id', 'desc')->get();

        return view('account.create')
            ->with(compact('account_types', 'accounts'));
    }
    ...
    public function edit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('account.edit')
                ->with(compact('account', 'account_types', 'accounts'));
        }
    }
...
//        dd($request->all());
        ...
        if (request()->ajax()) {
            try {
                ...
                $account = Account::where('business_id', $business_id)
                    ->findOrFail($id);
                $account->name = $input['en_name'] ?? '';
                $account->ar_name = $input['ar_name'] ?? '';
                $account->en_name = $input['en_name'] ?? '';
                $account->transaction_type = $input['transaction_type'] ?? null;
