    ...
    <div class="modal-body">
        <div class="row">
            <div class="col-lg-6">
                ...
                {!! Form::text('ar_name', $account->ar_name, ['class' => 'form-control', 'required','placeholder' => __( 'lang_v1.name' ) ]); !!}
            </div>

            <div class="col-lg-6">
                {!! Form::label('en_name', __( 'lang_v1.en_name' ) .":*") !!}
                {!! Form::text('en_name', $account->en_name, ['class' => 'form-control', 'required','placeholder' => __( 'lang_v1.en_name' ) ]); !!}
            </div>
        </div>
        <br/>
        ...
        <div class="form-group">
            ...
            <select name="transaction_type" class="form-control select2">
                <option {{$account->transaction_type == 'debtor' ? 'selected' : ''}} value="debtor">{{__('lang_v1.debtor')}}</option>
                <option {{$account->transaction_type == 'creditor' ? 'selected' : ''}} value="creditor">{{__('lang_v1.creditor')}}</option>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="parent_id" class="form-control select2">
                ...
                @foreach($accounts as $accountLoop)
                    @if(!($account->id == $accountLoop->id))
                        <option {{$account->parent_id == $accountLoop->id ? 'selected' : ''}} value="{{$accountLoop->id}}">{{app()->getLocale() === 'ar' ? $accountLoop->name : $accountLoop->en_name}} ({{$accountLoop->account_number}})</option>
                    @endif
                @endforeach
            </select>
        </div>
